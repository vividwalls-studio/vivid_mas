...
(function() {
  ...
  function updateTotalPrice() {
    ...
    if (state.galleryWrapChecked && variantSize && config.galleryWrapPrices[variantSize]) {
      additionalPrice = config.galleryWrapPrices[variantSize] * 100; // Convert to cents
      if (elements.galleryWrapPriceInput) {
        elements.galleryWrapPriceInput.value = config.galleryWrapPrices[variantSize];
      }
  ...
  function selectVariant(variantElement) {
    ...
    if (elements.skuDisplay) {
      ...
      elements.skuDisplay.textContent = `SKU: ${productHandle.toUpperCase()}-${size.toUpperCase()}`;
    }
  ...
  function handleThumbnailClick(event) {
    ...
    elements.thumbnails.forEach(thumb => {
      const indicator = thumb.querySelector('.vividwalls-selected-indicator');
      if (indicator) {
        indicator.remove();
      }
    });
    ...
    selectedIndicator.className = 'vividwalls-selected-indicator';
    selectedIndicator.style.cssText = 'width: 150px; height: 5px; background: #171717; border-radius: 5px; margin-bottom: 5px;';
    thumbnail.insertBefore(selectedIndicator, thumbnail.firstChild);
  }
  ...
  function initialize() {
    ...
    // Set initial states
    if (elements.galleryWrapCheckbox) {
      // Start with gallery wrap unchecked
